        ...
        try:
            ...
            for article in articles:
                search_index['articles'].append({
                    ...
                    'excerpt': self.truncate_text(article.subtitle or '', self.mobile_config['max_excerpt_length']),
                    'author': article.author_name,
                    'category': article.category_name,
                    'date': article.publication_date.isoformat() if article.publication_date else None,
                    'slug': article.slug,
                    'search_text': f"{article.title} {article.subtitle or ''} {article.author_name or ''} {article.category_name or ''}".lower()
                })
    ...
    def optimize_author_for_mobile(self, author: Author, include_articles: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if include_articles:
                ...
                author_articles = Article.find_by_author_slug(author.slug, limit=10)
    ...
    def optimize_category_for_mobile(self, category: Category, include_articles: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            if include_articles:
                ...
                category_articles = Article.find_by_category_slug(category.slug, limit=10)
